...
func (dao *StudentDao) FindByCustomerRule(ctx context.Context, namingInfo *gormdb.NameingSqlArgInfo, args any) (any, error) {
	...
	case "FindByWithPage":
		return dao.doFindByWithPage(ctx, namingInfo, args)
	default:
...
// doFindByWithPage 执行FindByWithPage查询（分页）
func (dao *StudentDao) doFindByWithPage(ctx context.Context, namingInfo *gormdb.NameingSqlArgInfo, args any) (*model.StudentFindByWithPagePageRes, error) {

	queryArgs, ok := args.(*model.StudentFindByWithPageArg)
	if !ok {
		return nil, errors.New("doFindByWithPage args type not match")
	}
	...
	oldwhere,_:=conditonwhere.ExtractWhereClauseByCut(execSql)
	newwhere,err:=queryArgs.FieldMask.BuildWhereFromConfig("FindByWithPage",model.StudentConditionMap)
	if err != nil {
		return nil, err
	}
	// 校验新的where条件是否使用了索引列，避免全表扫描
	...
	if !enablePage {
		return &model.StudentFindByWithPagePageRes{
			PageResult: gormdb.PageResult{
